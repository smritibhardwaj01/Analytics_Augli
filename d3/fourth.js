...
var container = d3.select("#fourth").append("svg").attr("width",150).attr("height",300)
                .attr("transform","translate(100,0)");

var colors = d3.scaleOrdinal().domain([]).range(["#4285f4","#db4437","#f4b400","#0f9d58","#ff6d00","#46bdc6"]);
...
var xScale = d3.scaleLinear().domain([]).range([0, 800]);
var yScale = d3.scaleLinear().domain([0,d3.max(bars)]).range([200, 0]);

var xAxis = d3.axisBottom(xScale);
var yAxis = d3.axisLeft(yScale);
    
var xContainer = d3.select("#fourth").append("svg").attr("width",800).attr("height",100)
        .attr("x",0)
        .attr("y",210)
        .attr("transform","translate(100,-225)");

var yContainer = d3.select("#fourth").append("svg").attr("width",35).attr("height",270)
                    .attr("x",0)
                    .attr("y",0)
                    .attr("transform","translate(-725,-315)");
...
function legend()
{
    var legend = d3.select("#fourth").append("svg").attr("width",150).attr("height",200)
                .attr("transform","translate(0,-400)");

    var key = ["Core (Grammar)","Pronunciation", "Vocabulary","Reading", "Assessment", "Summarization"];
    ...
    legend.selectAll("text")
        ...
        var legendColor = d3.scaleOrdinal().domain([]).range(["#4285f4","#db4437","#f4b400","#0f9d58","#ff6d00","#46bdc6"]);
...
d3.json("./fourth.json").then(

function(data)
{
    d3.select("#fourth").attr("transform","translate(100,0)");
    ...
    for(a in data)
    {
        ...
        if(dateDropdown == "Till Date")
            ...
            .on("change",function()
            {
                d3.select("#fourth").selectAll("svg").remove();
